...
public class AppBanco3 {
    private static ContaBancaria[] contas = 
        { new ContaBancaria ("Ana Luiza", 1000.00),
          new ContaEspecial("Maria", 800.00, 600.00),
          new ContaEspecial("Pedro", 450.00, 100.00) };
    ...
    private static void opcaoAumentarLimite(){
        double valorAdd= inputDouble("\n## Valor a ser adicionado ao limite de todas as especiais:");
        for (int i=0; i<contas.length; i++) {
            if (contas[i] instanceof ContaEspecial){ //instanceof verifica se a conta na posição i é uma instância da classe ContaEspecial
                double novoLimite = valorAdd + ((ContaEspecial) contas[i]).consultarLimite(); //operador cast converte tipo de classe do objeto
                ((ContaEspecial)contas[i]).alterarLimite(novoLimite);     
                print("\n## Novo Limite das contas especiais:" + ((ContaEspecial)contas[i]).consultarLimite());}
            }
        }
    }
